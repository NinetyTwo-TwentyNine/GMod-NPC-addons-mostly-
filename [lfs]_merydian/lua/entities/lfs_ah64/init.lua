...
function ENT:AltPrimaryAttack( Driver, Pod, Dir )
	...
	bullet.Spread 	= Vector( 0.0075,  0.0075, 0 )
	bullet.Tracer	= 1
...
function ENT:RunOnSpawn()
	self:GetDriverSeat().ExitPos = Vector(88,75,11)
	
	local GunnerSeat = self:AddPassengerSeat( Vector(95,0,-95), Angle(0,-90,7) )
	GunnerSeat.ExitPos = Vector(146,75,11)
...
function ENT:PrimaryAttack()
	...
	local Pos = self:LocalToWorld( Vector(14,84 * (self.FireLeft and 1 or -1),-106) )
	ent:SetPos( Pos )
...
function ENT:SecondaryAttack()
	...
	self:SetNextSecondary( 1.2 )
	
	self:EmitSound("TOW_FIRE")
	...
	local ent = ents.Create( "lunasflightschool_missile" )
	local Pos = self:LocalToWorld( Vector(22,55 * (self.FireLeft and 1 or -1),-111) )
	ent:SetPos( Pos )
	ent:SetAngles( (tr.HitPos - Pos):Angle() )
	ent:Spawn()
	ent:Activate()
...
function ENT:HandleWeapons(Fire1, Fire2, Fire3)
	...
	if self.OldFire3 ~= Fire3 then
		if Fire3 then
			self.wpn2 = CreateSound( self, "2a42_mi28_loop" )
			self.wpn2:Play()
			...
			if self.OldFire3 == true then
				...
				self:EmitSound( "2a42_mi28_lastshot" )
			end
		end
