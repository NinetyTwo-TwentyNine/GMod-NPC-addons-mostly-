...
function ENT:RunOnSpawn()
	...
	local GunnerSeat = self:AddPassengerSeat( Vector(121,0,44), Angle(0,-90,1) )
	GunnerSeat.ExitPos = Vector(121,70,0)
...
function ENT:PrimaryAttack()
	...
	local Pos = self:LocalToWorld( Vector(38,38 * (self.FireLeft and 1 or -1),28) )
	ent:SetPos( Pos )
...
function ENT:SecondaryAttack()
	...
	local ent = ents.Create( "lunasflightschool_missile" )
	local Pos = self:LocalToWorld( Vector(42,55 * (self.FireLeft and 1 or -1),28) )
	ent:SetPos( Pos )
	ent:SetAngles( (tr.HitPos - Pos):Angle() )
	ent:SetOwner( self )
	ent:Spawn()
...
function ENT:HandleWeapons(Fire1, Fire2, Fire3)
	...
	if Fire3 then
		...
		self.barrelSpinAdd = 45
	end
	
	if self.OldFire3 ~= Fire3 then
		if Fire3 then
			self.wpn2 = CreateSound( self, "MINIGUN_LOOP" )
			self.wpn2:Play()
			...
			if self.OldFire3 == true then
				...
				self:EmitSound( "MINIGUN_LASTSHOT" )
			end
		end
