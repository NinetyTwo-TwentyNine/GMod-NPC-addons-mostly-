	...
	if self.nextEFX < CurTime() then
		...
		if emitter then
			...
			for i = 0,1 do
				...
				local vOffset = self:LocalToWorld( Vector(-270,25 * Sub,85) )
...
function ENT:AnimLandingGear()
	...
	self:ManipulateBoneAngles( 1, Angle( -90,0,0) * gExp )
	self:ManipulateBoneAngles( 2, Angle( 180,0,0) * gExp )
	self:ManipulateBoneAngles( 3, Angle( -180,0,0) * gExp )
	self:ManipulateBoneAngles( 4, Angle( 90,0,0) * gExp )
	self:ManipulateBoneAngles( 5, Angle( 0,-90,0) * gExp )
	self:ManipulateBoneAngles( 6, Angle( 0,90,0) * gExp )
	self:ManipulateBoneAngles( 7, Angle( 0,0,-100) * gExp )
	
	self:ManipulateBonePosition( 8, Vector( 0,-81,0) * gExp )
	self:ManipulateBoneAngles( 8, Angle( -68,0,0) * gExp )
	
	self:ManipulateBonePosition( 9, Vector( 0,81,0) * gExp )
	self:ManipulateBoneAngles( 9, Angle( 68,0,0) * gExp )
