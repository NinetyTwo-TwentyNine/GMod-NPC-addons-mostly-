	...
	if ent:GetClass() == "npc_apcdriver" then
		...
		if IsValid(ent) && ent:MapCreationID() == -1 then
			if !table.IsEmpty(ents.FindByName(ent:GetKeyValues()["vehicle"])) then
				...
				timer.Create("DrivingCombineAPC"..ent:EntIndex(), 0.1, 0, function()
				if IsValid(ent) && IsValid(ent.Vehicle) && GetConVarNumber("ai_disabled") == 0 then
//==============================================================================================================================================================================
					ent.IsInWater = (ent.Vehicle:WaterLevel() > 2)
					ent.NoGroundConnection = (ent.Vehicle:GetOperatingParams().wheelsInContact <= 1)
					ent.HasTarget = IsValid(ent:GetEnemy())
					ent.TargetInSight = ent.HasTarget && ent:Visible(ent:GetEnemy())
//==============================================================================================================================================================================
					if ent.Driving then
						if !ent:IsMoving() then
							if ent.IsStuck then
								ent.IsStuck = false
								ent.StuckTimer = nil
							...
							ent.Driving = false
						elseif ent.Vehicle:GetSpeed() == 0 then
							...
							if ent.StuckTimer >= 1.6 then
								if !ent.IsStuck then
									...
									if (ent.Vehicle:GetPos() + ent.Vehicle:GetForward()*ent.Vehicle:OBBMins().y):Distance(ent:GetCurWaypointPos()) > (ent.Vehicle:GetPos() + ent.Vehicle:GetForward()*ent.Vehicle:OBBMaxs().y):Distance(ent:GetCurWaypointPos()) then
										ent.StuckDistance = ent.Vehicle:OBBMins().y
									else
										ent.StuckDistance = ent.Vehicle:OBBMaxs().y
									end
									table.insert( ent.VehiclePath, navmesh.GetNearestNavArea(ent.Vehicle:GetPos() + ent.Vehicle:GetForward()*ent.StuckDistance) )
									...
									if ent.CurrentArea >= table.Count(ent.VehiclePath) then
										...
										ent:Fire("Stop")
										print(tostring(ent)..": This APC is totally stuck... Welp, now I'm stationary!")
									end
								end
					...
					if ent.HasTarget then
						if !ent.Driving and !ent.TargetInSight then
							if !ent.VehiclePath then
								ent.VehiclePath = Astar( navmesh.GetNearestNavArea(ent.Vehicle:GetPos()), navmesh.GetNearestNavArea(ent:GetEnemyLastKnownPos()), math.abs(ent.Vehicle:OBBMaxs().x - ent.Vehicle:OBBMins().x), ent.AvoidAreas )
								ent.CurrentArea = 1
								...
								if table.GetLastValue(ent.VehiclePath) != navmesh.GetNearestNavArea(ent:GetEnemyLastKnownPos()) then
									if table.HasValue( ent.VehiclePath, navmesh.GetNearestNavArea(ent:GetEnemyLastKnownPos()) ) then
										...
										if table.HasValue( ent.CheckTable, navmesh.GetNearestNavArea(ent:GetEnemyLastKnownPos()) ) then
											ent.PathBuildup = Astar( ent.VehiclePath[table.Count(ent.VehiclePath)], navmesh.GetNearestNavArea(ent:GetEnemyLastKnownPos()), math.abs(ent.Vehicle:OBBMaxs().x - ent.Vehicle:OBBMins().x), ent.AvoidAreas )
...
//==============================================================================================================================================================================
				...
				ent:CallOnRemove("DestroyTheTimer", function()
					timer.Remove("DrivingCombineAPC"..ent:EntIndex())
					if IsValid(ent.DrivePoint) then
						ent.DrivePoint:Remove()
					end
				end)
