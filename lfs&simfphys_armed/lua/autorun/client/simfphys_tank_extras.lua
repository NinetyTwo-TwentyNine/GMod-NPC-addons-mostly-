...
local function UpdatePumaScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("p_trackmat_"..id.."_left", "VertexLitGeneric", { ["$basetexture"] = "models/apc/ger/puma/track", ["$alphatest"] = "1", ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end

	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("p_trackmat_"..id.."_right", "VertexLitGeneric", { ["$basetexture"] = "models/apc/ger/puma/track", ["$alphatest"] = "1", ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end
	
	local TrackPos = GetTrackPos( ent, 80, 1 )
	
	ent.wheel_left_mat:SetVector("$translate", Vector(0,TrackPos.Left,0) )
	ent.wheel_right_mat:SetVector("$translate", Vector(0,TrackPos.Right,0) )

	ent:SetSubMaterial( 2, "!p_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 3, "!p_trackmat_"..id.."_right" )
end

local function UpdateFV510ScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("fv510_trackmat_"..id.."_left", "VertexLitGeneric", { ["$basetexture"] = "models/fv510warrior_shelby/uk_fv510_isv_track_c", ["$alphatest"] = "1", ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end

	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("fv510_trackmat_"..id.."_right", "VertexLitGeneric", { ["$basetexture"] = "models/fv510warrior_shelby/uk_fv510_isv_track_c", ["$alphatest"] = "1", ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end
	
	local TrackPos = GetTrackPos( ent, -400, 1 )
	
	ent.wheel_left_mat:SetVector("$translate", Vector(0,TrackPos.Left,0) )
	ent.wheel_right_mat:SetVector("$translate", Vector(0,TrackPos.Right,0) )

	ent:SetSubMaterial( 8, "!fv510_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 9, "!fv510_trackmat_"..id.."_right" )
end
...
local function UpdateChallengerScrollTexture( ent )
	local id = ent:EntIndex()

	if not ent.wheel_left_mat then
		ent.wheel_left_mat = CreateMaterial("c_trackmat_"..id.."_left", "VertexLitGeneric", { ["$basetexture"] = "models/challenger_mk3_shelby/uk_challenger_1_track_c", ["$alphatest"] = "1",  ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end

	if not ent.wheel_right_mat then
		ent.wheel_right_mat = CreateMaterial("c_trackmat_"..id.."_right", "VertexLitGeneric", { ["$basetexture"] = "models/challenger_mk3_shelby/uk_challenger_1_track_c", ["$alphatest"] = "1", ["$translate"] = "[0.0 0.0 0.0]", ["Proxies"] = { ["TextureTransform"] = { ["translateVar"] = "$translate", ["centerVar"]    = "$center",["resultVar"]    = "$basetexturetransform", } } } )
	end
	
	local TrackPos = GetTrackPos( ent, -90, 1 )
	ent.wheel_left_mat:SetVector("$translate", Vector(0,TrackPos.Left,0) )
	ent.wheel_right_mat:SetVector("$translate", Vector(0,TrackPos.Right,0) )

	ent:SetSubMaterial( 8, "!c_trackmat_"..id.."_left" ) 
	ent:SetSubMaterial( 9, "!c_trackmat_"..id.."_right" )
end

local TrackData = {
	sim_fphys_spz_puma_armed =  function( ent ) UpdatePumaScrollTexture( ent ) end,
	sim_fphys_fv510_armed =  function( ent ) UpdateFV510ScrollTexture( ent ) end,
	...
	sim_fphys_tank5 = function( ent ) UpdateChallengerScrollTexture( ent ) end,
}
