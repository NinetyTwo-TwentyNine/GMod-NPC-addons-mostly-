...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	vehicle:EmitSound("FV510WARRIOR_CANNON_FIRE")
...
local function atgm_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	missile:SetSaveValue( "m_flDamage", 300 )
	missile:Spawn()
	missile:Activate()
	...
	vehicle:EmitSound("simulated_vehicles/weapons/tiger_reload.wav", 70, math.random(160, 180), 1, CHAN_BODY)
end
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "cannon_muzzle", Direction = Vector(0, 0.5, 0), Type = 4 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(0,0,7), Vector(0,70,80), true, "gunner_sight" )
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	local ID2 = vehicle:LookupAttachment( "turret_att" )
	...
	if ply:KeyDown( IN_RELOAD ) and vehicle.CurMag < vehicle.MaxMag and self:CanPrimaryAttack(vehicle) then
		vehicle.CurMag = vehicle.MaxMag
		vehicle:EmitSound("FV510WARRIOR_RELOAD")
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	local effectdata = EffectData()
		...
		effectdata:SetAttachment( vehicle:LookupAttachment( "cannon_muzzle" ) )
...
function simfphys.weapon:SecondaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	local shootOrigin_deltapos,_ = LocalToWorld( Vector(0, -50, 10), Angle(), Vector(0,0,0), Attachment.Ang )
	shootOrigin = shootOrigin + shootOrigin_deltapos
	local shootDirection = Attachment.Ang:Forward()
...
/*
	local effectdata = EffectData()
		...
		effectdata:SetAngles( Attachment.Ang )
		effectdata:SetEntity( vehicle )
		effectdata:SetAttachment( vehicle:LookupAttachment( "turret_att" ) )
		effectdata:SetScale( 15 )
	util.Effect( "CS_MuzzleFlash", effectdata, true, true )
*/

	self:SetNextSecondaryFire( vehicle, CurTime() + 3.0 )
end
...
function simfphys.weapon:CanSecondaryAttack( vehicle )
	...
	return (vehicle.NextShoot2 < CurTime()) && (table.Count(vehicle.MissileTracking) < vehicle.MaxRockets)
end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i,v in pairs( fv510_SusData ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( fv510_SusData[i].Attachment ) ).Pos + Up * 15
		...
		vehicle:SetPoseParameter(fv510_SusData[i].PoseParameter, vehicle.oldDist[i] )
	end
end
