...
local challenger_SusData = {}
for i = 1,6 do
	challenger_SusData[i] = { 
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(5,0,5), Vector(0,70,80), true, "gunner_sight" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "hmg_muzzle", Type = 5 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(0,0,0), Vector(0,0,50), true, "hmg_sight" )
...
function simfphys.weapon:AimMachinegun( ply, vehicle, pod )	
	...
	vehicle:SetPoseParameter("hmg_aim_yaw", TargetAng.y - vehicle.sm_pp_yaw )
	vehicle:SetPoseParameter("hmg_aim_pitch", -TargetAng.p )
end
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	local pclamp = math.Clamp( (math.cos( math.rad(TargetAng.y + 180) ) - 0.7) * 6,0,1) ^ 2 * 15
...
function simfphys.weapon:ControlMachinegun( vehicle, deltapos )
	...
	local ID = vehicle:LookupAttachment( "hmg_muzzle" )
	local Attachment = vehicle:GetAttachment( ID )
...
function simfphys.weapon:SecondaryAttack( vehicle, ply, deltapos, cPos, cAng )
	...
	local ID = vehicle:LookupAttachment( "coax_muzzle" )
	local Attachment = vehicle:GetAttachment( ID )
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( challenger_SusData ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( challenger_SusData[i].Attachment ) ).Pos + Up * 15
		...
		local Dist = (pos - trace.HitPos):Length() - 35
		...
		vehicle:SetPoseParameter(challenger_SusData[i].PoseParameter, vehicle.oldDist[i] )
	end
end
