...
hook.Add( "PlayerInitialSpawn", "!!!!lfsPlayerTeamSetup", function( ply )
	timer.Simple( FrameTime(), function() 
		if not IsValid( ply ) then return end

		ply:lfsSetAITeam(simfphys.LFS.PlayerDefaultTeam:GetInt())
	end )
end )

hook.Add( "PlayerSpawnedSENT", "!!!!lfsPlaneTeamSetup", function( ply, ent )
	timer.Simple( FrameTime(), function() 
		if not IsValid( ent ) || not IsValid( ply ) then return end

		if ent.LFS || ent.LVS then
			if ply:lfsGetAITeam() ~= 0 then
				ent:SetAITEAM(ply:lfsGetAITeam())
			end
		end
	end )
end )

hook.Add( "AcceptInput", "LFS AI handling: If NPC's team gets changed", function( ent, input, activator, caller, value )
	if ent:GetClass() ~= "npc_vehicledriver" then return end

	local vehicle = ent.Vehicle
	...
	if string.lower(input) == "addoutput" then
		...
		timer.Simple(FrameTime(), function()
			...
			if prev_team ~= cur_team then
				vehicle:SetAITEAM(cur_team)
			end
		end)
	end
end)

hook.Add( "EntityKeyValue", "LFS AI handling: If NPC's team gets changed", function( ent, key, value )
	if ent:GetClass() ~= "npc_vehicledriver" then return end
...
hook.Add( "EntityRemoved", "LFS AI handling: If the NPC gets removed", function( ent )
	if ent:GetClass() ~= "npc_vehicledriver" then return end
...
if SERVER then 
	...
	function meta:lfsSetAITeam( nTeam )
		...
		if self:lfsGetAITeam() ~= nTeam then
			self:PrintMessage( HUD_PRINTTALK, "[LFS] Your AI-Team has been updated to: "..(TeamText[ nTeam ] or ("Team "..nTeam)) )
			self:SetTeam( nTeam )
		end
	end
	...
	hook.Add( "PlayerEnteredVehicle", "!!LFS_Enter", function( ply, vehicle )
		...
		if ply:lfsGetAITeam() ~= 0 && Parent:GetAITEAM() ~= 0 then
			if simfphys.LFS.FreezeTeams:GetBool() then
				Parent:SetAITEAM( ply:lfsGetAITeam() )
			else
...
if CLIENT then
	...
	local cvarHitMarker = CreateConVar( "lfs_hitmarker", 1, 0, "")
