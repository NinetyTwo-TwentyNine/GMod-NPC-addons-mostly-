...
function ENT:InitializeVehicle()
	...
	self.DriverSeat:GetPhysicsObject():AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
	...
	self.DriverSeat:AddEFlags( EFL_DONTBLOCKLOS )
	...
	if self.PassengerSeats then
		for i = 1, table.Count( self.PassengerSeats ) do
			...
			self.pSeat[i]:AddEFlags( EFL_DONTBLOCKLOS )
			...
			self.pSeat[i]:GetPhysicsObject():AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
	...
	if self.Attachments then
		for i = 1, table.Count( self.Attachments ) do
			...
			local pobj = prop:GetPhysicsObject()
...
function ENT:SetupVehicle()
	...
	self.MassOffset:GetPhysicsObject():AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
	...
	self.MassOffset:AddEFlags( EFL_DONTBLOCKLOS )
	...
	if self.CustomWheels then
		if self.CustomWheelModel then
			...
			if self.SteerFront ~= false then
				...
				self.SteerMaster:AddEFlags( EFL_DONTBLOCKLOS )
			...
			if self.SteerRear then
				...
				self.SteerMaster2:AddEFlags( EFL_DONTBLOCKLOS )
