...
if SERVER then
	hook.Add("EntityTakeDamage", "Damage link from wheels to the vehicle itself", function(target, dmginfo)
		if IsValid(dmginfo:GetAttacker()) then
			if dmginfo:GetAttacker():GetClass() == "gmod_sent_vehicle_fphysics_wheel" then
				if IsValid(dmginfo:GetAttacker():GetBaseEnt()) then
					dmginfo:SetInflictor(dmginfo:GetAttacker())
					dmginfo:SetAttacker(dmginfo:GetAttacker():GetBaseEnt())
				end
			end
		end
	end)
