...
function ENT:Initialize()
	...
	self:AddEFlags( EFL_DONTBLOCKLOS ) -- this allows NPCs to see the driver inside this entity
	...
	PObj:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
...
function ENT:Use( ply )
	...
	if self:GetlfsLockedStatus() or self:HasPassengerEnemyTeam( ply ) then 
...
function ENT:HasPassengerEnemyTeam( ply )
	if not simfphys.LFS.TeamPassenger:GetBool() then return false end
	
	if not IsValid( ply ) then return true end
	
	local myteam = ply:Team()
	if myteam == 0 then return false end


	if self:GetAI() then
		if self:GetAITEAM() ~= 0 && self:GetAITEAM() ~= myteam then
			return true
		end
	end

	if IsValid( self:GetDriver() ) then
		if self:GetDriver():Team() ~= 0 && self:GetDriver():Team() ~= myteam then
			return true
		end
	end
	
	if self:GetPassengerSeats() then
		for i = 1, table.Count( self.pSeats ) do
			if IsValid(self.pSeats[i]) then
				
				local Passenger = self.pSeats[i]:GetDriver()
				if IsValid( Passenger ) then
					if Passenger:lfsGetAITeam() ~= 0 && Passenger:lfsGetAITeam() ~= myteam then
						return true
					end
				end
			end
	...
	if isnumber( self.WheelMass ) and isnumber( self.WheelRadius ) then
		if isvector( self.WheelPos_L ) then
			...
			if IsValid( wheel_L ) then
				...
				wheel_L:AddEFlags( EFL_DONTBLOCKLOS )
				...
				LWpObj:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
		...
		if isvector( self.WheelPos_R ) then
			...
			if IsValid( wheel_R ) then
				...
				wheel_R:AddEFlags( EFL_DONTBLOCKLOS )
				...
				RWpObj:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
		...
		if isvector( self.WheelPos_C ) then
			...
			if IsValid( SteerMaster ) then
				...
				local smPObj = SteerMaster:GetPhysicsObject()
				if IsValid( smPObj ) then
					smPObj:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
					smPObj:EnableMotion( false )
				end
				...
				SteerMaster:AddEFlags( EFL_DONTBLOCKLOS )
				...
				if IsValid( wheel_C ) then
					...
					wheel_C:AddEFlags( EFL_DONTBLOCKLOS )
					...
					CWpObj:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
...
function ENT:InitPod( Pos, Ang )
	...
	if not IsValid( Pod ) then
		...
		Pod:AddEFlags( EFL_DONTBLOCKLOS )
		--Pod:SetNoDraw( true )
		...
		if IsValid( DSPhys ) then
			DSPhys:AddGameFlag( FVPHYSICS_MULTIOBJECT_ENTITY )
...
function ENT:OnTakeDamage( dmginfo )
	...
	local ShieldCanBlock = false
	local Damage = dmginfo:GetDamage()
	if dmginfo:IsDamageType( DMG_BLAST ) || dmginfo:IsDamageType( DMG_BLAST_SURFACE ) then 
		Damage = Damage * 7.5
	elseif dmginfo:IsDamageType( DMG_AIRBOAT ) || dmginfo:IsDamageType( DMG_DIRECT ) then
		ShieldCanBlock = self.IsArmored
		Damage = Damage * 1
...
function ENT:Destroy()
	...
	if IsValid(self:AIGetSelf()) then
		self:AIGetSelf():Remove()
	end
...
function ENT:CreateAI()
	if !IsValid(self:AIGetSelf()) then
		self.NPC = ents.Create("npc_vehicledriver")
		self:GetDriverSeat():SetName("LFS["..self:EntIndex().."]driver_seat")
		self.NPC:SetKeyValue("vehicle", self:GetDriverSeat():GetName())
		self.NPC:SetKeyValue("teamnum", tostring(self:GetAITEAM()))
		self.NPC:SetKeyValue("targetname", "LFS["..self:EntIndex().."]AI")
		self.NPC.Vehicle = self
		self.NPC:Spawn()
		self.NPC:Activate()

		if self:IsHelicopter() then
			self.NPC:Fire("setmaxlookdistance", 8192)
		else
			self.NPC:Fire("setmaxlookdistance", 16384)
		end

		local seat_pos = self:GetDriverSeat():GetPos()
		local seat_ang = self:GetDriverSeat():GetAngles()
		self:GetDriverSeat():SetParent(NULL) -- !!!hack!!! So, for some reason when an NPC is inside a seat that has a parent, its target field becomes half-broken, I've got no idea.
		self:GetDriverSeat():SetPos(seat_pos)
		self:GetDriverSeat():SetAngles(seat_ang)
		constraint.Weld(self:GetDriverSeat(), self, 0, 0, 0, false, false)
		self:GetDriverSeat():GetPhysicsObject():EnableMotion(true)
	end
...
function ENT:RemoveAI()
	...
	self:GetDriverSeat():SetName("")
	constraint.RemoveConstraints(self:GetDriverSeat(), "Weld")
	self:GetDriverSeat():SetParent(self)
...
function ENT:OnAITEAMSwitched(name, old, new)
	if new == old then return end
	
	if IsValid(self:AIGetSelf()) then
		local aiteam = self:AIGetSelf():GetInternalVariable("TeamNum")
		if new != aiteam then
			self:AIGetSelf():Fire("AddOutput", "TeamNum "..tostring(new))
		end
	end
end
...
function ENT:AIGetSelf()
	return self.NPC
end

function ENT:AIGetTarget()
	return self:AIGetSelf():GetEnemy()
end

function ENT:AIGetRelationship(ent)
	if !IsValid(ent) then return D_NU end
	return self:AIGetSelf():Disposition(ent)
end

function ENT:AICanSee( otherEnt )
	if not IsValid( otherEnt ) then return false end
	return self:AIGetSelf():IsLineOfSightClear( otherEnt )
end
		...
		if self:GetStability() < 0.3 then
			...
			if alt > mySpeed then
				...
				if IsValid( Target ) && self:AICanSee( Target ) then
					if self:AITargetInfront( Target, 100 ) then
						...
						if alt > 6000 and self:AITargetInfront( Target, 54 ) then
