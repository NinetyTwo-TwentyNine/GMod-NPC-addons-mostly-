...
if(CLIENT) then
	...
	CreateClientConVar("npctool_spawner_spawninputs_select","",true)
	...
	local function MainMenu(pnl)
		...
		pCBox.OnSelect = function(pCBox,idx,val,data)
			...
			if(data.spawninputs) then tbSpawnInputs = data.spawninputs end
		...
		b.OnMousePressed = function(b)
			CreateSaveDialog("NPC Spawner Settings",function(name)
				...
				data.spawninputs = tbSpawnInputs
		...
		for k,data in pairs(list.Get("NPCUsableWeapons")) do
			options[data.title] = {npctool_spawner_equipment = data.class}
		end
		...
		local options = {}
		for input_name,input_param in pairs(tbSpawnInputs) do
			if input_param != "" then
				options[input_name .. " : " .. input_param] = {npctool_spawner_spawninputs_select = input_name}
			else
				options[input_name] = {npctool_spawner_spawninputs_select = input_name}
			end
		end
		pnl:AddControl("ListBox",{Label = "Spawn Inputs",MenuButton = 0,Height = 150,Options = options})
		pnl:AddControl("Button",{Label = "Add Spawn Input",Text = "Add Spawn Input",Command = "npctool_spawner_spawninputs_add"})
		pnl:AddControl("Button",{Label = "Remove Spawn Input",Text = "Remove Spawn Input",Command = "npctool_spawner_spawninputs_remove"})
		pnl:AddControl("Button",{Label = "Clear Spawn Inputs",Text = "Spawn Inputs",Command = "npctool_spawner_spawninputs_clear"})
	end
	...
	concommand.Add("npctool_spawner_spawninputs_remove",function(pl,cmd,args)
		local sel = GetConVarString("npctool_spawner_spawninputs_select")
		tbSpawnInputs[sel] = nil
		MainMenu(controlpanel.Get("npctool_spawner"))
	end)
	concommand.Add("npctool_spawner_spawninputs_clear",function(pl,cmd,args)
		tbSpawnInputs = {}
		MainMenu(controlpanel.Get("npctool_spawner"))
	end)
	concommand.Add("npctool_spawner_spawninputs_add",function(pl,cmd,args)
		local input_name = args[1]
		local input_param = args[2]
		if(input_name && input_param) then
			tbSpawnInputs[input_name] = input_param
			MainMenu(controlpanel.Get("npctool_spawner"))
			return
		end
		local w,h = 175,145
		...
		local teInputName = vgui.Create("DTextEntry",p)
		teInputName:SetSize(100,16)
		teInputName:SetPos(70,35)
		...
		local teInputParam = vgui.Create("DTextEntry",p)
		teInputParam:SetSize(100,16)
		teInputParam:SetPos(70,60)
		...
		b.DoClick = function(b)
			p:Close()
			local input_name = teInputName:GetValue()
			local input_param = teInputParam:GetValue()
			if(input_name != "") then
				RunConsoleCommand("npctool_spawner_spawninputs_add",input_name,input_param)
			end
		end
	...
	net.Receive("cl_npctool_spawner_spawn",function(len)
		...
		net.Start("sv_npctool_spawner_spawn")
			...
			net.WriteUInt(table.Count(tbSpawnInputs),8)
			for input_name,input_param in pairs(tbSpawnInputs) do
				net.WriteString(input_name)
				net.WriteString(input_param)
			end
		net.SendToServer()
	end)
	...
	net.Receive("sv_npctool_spawner_spawn",function(len,pl)
		...
		local numSpawnInputs = net.ReadUInt(8)
		local tbSpawnInputs = {}
		for i = 1,numSpawnInputs do
			local input_name = net.ReadString()
			local input_param = net.ReadString()
			tbSpawnInputs[input_name] = input_param
		end
		/*
		...
		*/
		...
		ent:SetNPCSpawnInputs(tbSpawnInputs)
