...
local ent = ents.Create( "npc_civil_protection_tier2_dropship" )
...
self.npc:SetName( "Tier 2 Civil Protection Dropship")
...
timer.Simple(2,function()
	...
	undo.Create( "Tier 2 Civil Protection Dropship" )
	undo.AddEntity( DropShip_Corpse )	
	undo.SetCustomUndoText( "Undone Tier 2 Civil Protection Dropship" )
...
local npc = self.npc
	...
	if npc:GetNWBool("HasTroops") == true and IsValid(Crate) and Crate:GetParent() == npc and npc:GetName() == "Tier 2 Civil Protection Dropship" and Crate:GetName() == "dropship_container" and not self.SquadDeploying then
...
local Open_Idle = self.container:LookupSequence( "open_idle" )
						...
						self.SquadDeploying = true
							...
							self.Metro_Cop1:SetKeyValue( "squadname", "Tier 2 Civil Protection".. self.Metro_Cop1:EntIndex() )
						...
						self.Metro_Cop1:SetName( Metro_Cop1_name )
							...
							self.Metro_Cop2:SetKeyValue( "squadname", "Tier 2 Civil Protection".. self.Metro_Cop1:EntIndex() )
							end
						...
						self.Metro_Cop2:SetName( Metro_Cop2_name )
							...
							if IsValid(self.Metro_Cop1) then
							self.Metro_Cop3:SetKeyValue( "squadname", "Tier 2 Civil Protection".. self.Metro_Cop1:EntIndex() )
							end
							self.Metro_Cop3:Give( "weapon_smg1" )
						...
						self.Metro_Cop3:SetName( Metro_Cop3_name )
							...
							if IsValid(self.Metro_Cop1) then
							self.Metro_Cop4:SetKeyValue( "squadname", "Tier 2 Civil Protection".. self.Metro_Cop1:EntIndex() )
							end
						...
						self.Metro_Cop4:SetName( Metro_Cop4_name )
							...
							if IsValid(self.Metro_Cop1) then
							self.Metro_Cop5:SetKeyValue( "squadname", "Tier 2 Civil Protection".. self.Metro_Cop1:EntIndex() )
							end
						...
						self.Metro_Cop5:SetName( Metro_Cop5_name )
							...
							timer.Simple(15,function()
								if IsValid(self.container) then
							self.Metro_Cop6 = ents.Create( "npc_metropolice" )
							self.Metro_Cop6:SetPos( self.container:GetPos() + self.container:GetForward()*-26 + self.container:GetUp()*-36 )
							self.Metro_Cop6:SetKeyValue( "spawnflags", "644" )
							if IsValid(self.Metro_Cop1) then
							self.Metro_Cop6:SetKeyValue( "squadname", "Tier 2 Civil Protection" .. self.Metro_Cop1:EntIndex() )
							end
							self.Metro_Cop6:SetKeyValue( "manhacks", "1" )
							self.Metro_Cop6:Give( "weapon_smg1" )
							self.Metro_Cop6:Spawn()
							self.Metro_Cop6:Activate()
						local Metro_Cop6_name = "DeployingCop6" .. self.Metro_Cop6:EntIndex()
						self.Metro_Cop6:SetName( Metro_Cop6_name )
							self.SequencePlayed = false
							self.Metro_Cop6_Sequence = ents.Create( "scripted_sequence" )
							self.Metro_Cop6_Sequence:SetName( Metro_Cop6_name .. "_wake_seq" )
							self.Metro_Cop6_Sequence:SetKeyValue( "spawnflags", "624" )
							self.Metro_Cop6_Sequence:SetKeyValue( "m_iszEntity", Metro_Cop6_name )
							self.Metro_Cop6_Sequence:SetKeyValue( "m_iszIdle", "batonidle1" )
							self.Metro_Cop6_Sequence:SetKeyValue( "m_fMoveTo", "4" )
							self.Metro_Cop6_Sequence:SetKeyValue( "m_iszPlay", "Dropship_Deploy" )
							self.Metro_Cop6_Sequence:SetPos( self.container:GetPos() + self.container:GetForward()*-26 + self.container:GetUp()*-36 )
							self.Metro_Cop6_Sequence:Spawn()
							self.Metro_Cop6_Sequence:Activate()
							self.Metro_Cop6_Sequence:SetParent( self.Metro_Cop6 )
								if self.SequencePlayed == false then
								self.Metro_Cop6_Sequence:Fire( "BeginSequence", "", 0 )
								self.SequencePlayed = true
								timer.Simple(2.6666666666667,function()
									if IsValid(self.Metro_Cop5) and self.SequencePlayed and IsValid(self.container) and not self.Metro_Cop5:IsCurrentSchedule(SCHED_FORCED_GO_RUN) then
									self.Metro_Cop5:ExitScriptedSequence()
...
if ent:GetName() == "Tier 2 Civil Protection Dropship" and dmginfo:IsDamageType(DMG_BLAST) and ent:Health() > 0 then
